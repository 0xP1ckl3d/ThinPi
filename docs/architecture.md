...
`/args-from:stdin` argument; its structured connection parameters, explicit
non-interactive certificate policy and password are newline-delimited on stdin.
TigerVNC receives an ephemeral mode-0600
...
never placed in argv. Exiting the remote shell destroys xterm.
