...
ApplicationWindow {
    ...
    ErrorDialog { visible: backend.errorMessage.length > 0 && backend.view !== "offline"; message: backend.errorMessage; confirmSSHHostKey: backend.sshHostKeyConfirmation; confirmAudioUnavailable: backend.audioUnavailableConfirmation; onClosed: { if (!backend.sshHostKeyConfirmation && !backend.audioUnavailableConfirmation) backend.dismissError() } }
